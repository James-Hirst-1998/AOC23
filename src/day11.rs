...
fn calculate_distance_between_galaxies(contents: String, expansion_constant: u64) -> Vec<u64> {
    ...
    for (row1, col1) in galaxy_positions.iter(){
        for (row2, col2) in galaxy_positions.iter(){
            ...
            let row_distance = if row1 > row2 {*row1 - *row2} else {*row2 - *row1};
            let col_distance = if col1 > col2 {*col1 - *col2} else {*col2 - *col1};
...
impl Solution for Day11 {
    ...
    fn part_one(_parsed_input: &mut Self::ParsedInput) -> String {
        let total_distance = calculate_total_distance_between_galaxies(_parsed_input.to_string(), 1);
        total_distance.to_string()
    }

    fn part_two(_parsed_input: &mut Self::ParsedInput) -> String {
        let total_distance = calculate_total_distance_between_galaxies(_parsed_input.to_string(), 999999);
        total_distance.to_string()
    }
}
...
mod tests {
    ...
    fn check_day11_both_case1() {
        ...
        ..........
        .......#..
        #...#.....", false), ("374".to_string(), "82000210".to_string()))
    }
}
